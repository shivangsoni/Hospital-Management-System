...
$query = $DBcon->query("SELECT  p_email_id,p_password FROM rooms,rooms_patient,patient WHERE rooms_patient.P_ID=patient.P_ID and rooms.H_ID=rooms_patient.H_ID  and p_email_id=".$_SESSION['userSession']);
...
<title>Welcome - <?php echo $userRow['P_Name']; ?></title>
...
// Perform queries 
if($query_sql=mysqli_query($con,"SELECT  D_name,doctor.D_cat  FROM doctor,rooms_patient WHERE rooms_patient.p_prob=doctor.D_cat"))
{
	echo '<br/>connected';
}
else
...
//VALUES ('Glenn','Quagmire',33)");
 ...
 {
echo "<table border='1'>";
echo "<tr><th>'D_cat'</th> <th>'D_name'</th></tr>";
...
?>
      ...
      <h4> ENTER YOUR DOCTOR NAME YOU WANT TO GET TREATED WITH:</h4>
       <SELECT NAME = "grade"> 
       <OPTION SELECTED > karunakar
       <OPTION> manoj
       <OPTION> nalin
       <OPTION> sonu
       <OPTION> raj
